...
export function findQuestion(
    ...
    const specificQuestion: Question[] = questions.filter(
        (question: Question): boolean => question.id == id
    );
    if (!specificQuestion.length) {
        return null;
    } else {
        return specificQuestion[0];
    }
}
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    const removedQuestion: Question[] = questions.filter(
        (question: Question): boolean => question.id != id
    );
...
export function sumPoints(questions: Question[]): number {
    const sumOfPoints: number = questions.reduce(
...
export function sumPublishedPoints(questions: Question[]): number {
    const sumOfPoints: number = questions
        ...
        questions
            .map(
                (question: Question): string =>
                    ...
                    question.name +
                    "," +
                    (!question.options ? "0" : question.options.length) +
                    "," +
                    question.points +
                    "," +
                    question.published
